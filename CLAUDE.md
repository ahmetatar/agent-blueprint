...
- `editor/` — `abp editor`: local FastAPI server + embedded React UI (read-only visualizer, phase E1). `viewmodel.py` builds the canvas graph from `BlueprintSpec` (NOT the flattened IR — subgraphs stay groups) reusing the compiler's `_resolve_llm`; `diagnostics.py` maps lint locations to YAML line/col via ruamel `lc` data; `/ws` pushes `file_changed` from a watchfiles watcher. Frontend lives in `frontend/` (Vite + React Flow + ELK + Monaco); `npm run build` outputs into `editor/static/` (VCS-ignored, wheel-embedded via hatch hook). Monaco is bundled via the slim `editor.api` entry — the stock `yaml.contribution` lazy import breaks vite's import analysis, so YAML is registered statically in `monacoSetup.ts`.
